...
def checkUpdate(config):
    ...
    if (r.status_code == requests.codes.ok):
        if (r.text.split("\n")[0] == version):
            ...
            r = requests.get(url)
            if (r.status_code == requests.codes.ok):
                f = open("/home/slave-nafsdm/pythondaemon/tempUpgrade/temp_upgrade.sh")
                f.write(r.content)
                f.close()
                import subprocess
                ...
                r = requests.get(url)
                if (r.status_code == requests.codes.ok):
                    f = open("/home/slave-nafsdm/pythondaemon/tempUpgrade/temp_upgrade.py")
                    f.write(r.content)
                    f.close()
                    import subprocess
                    ...
                    if upgradeStatus == "exception":
                        ...
                        f = open("/home/slave-nafsdm/upgradeLog.log")
